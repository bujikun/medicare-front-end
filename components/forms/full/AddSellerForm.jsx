...
const AddSellerForm = () => {
  ...
  const onSubmit = async (values, { setSubmitting }) => {
    setSubmitting(true);
    setIsLoading(true);
    const response = await fetch("/api/admin/sellers/add", {
      method: "POST",
    ...
    >
      <Box w={{ base: "300px", md: "400px", lg: "500px" }}>
        ...
        {isFailed && (
          <Alert status="error">
            <AlertTitle>Error!</AlertTitle>
            <AlertDescription>Seller could not be saved.Try again later</AlertDescription>
          </Alert>
        )}
        <Formik
        ...
        >
          <Form method="POST">
            <VInputField
              ...
              error={error}
            />
            <VSubmitButton text="Add" isLoading={isLoading}  loadingText="Adding Seller..."/>
          </Form>
        </Formik>
      </Box>
    </Flex>
